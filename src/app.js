...
//called on stripe/razorpay payment success
async function creditAndNotify(txId,stripeOrderId){
  ...
  sendEmailNotification(depositor.email, 'Deposit Received',`<p>Your account was credited with <b>₹${tx.amount}</b>.</p>`, depositor._id, 'deposit');

  if(depositor.notifyWhatsApp && depositor.phone){
    sendWhatsAppNotification(depositor.phone,`Your account was credited with ₹${tx.amount}.`, null, depositor._id, 'deposit');
  }
}
