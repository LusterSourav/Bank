...
// ponytail: FATF R.16 — logs remit originator/beneficiary info for transfers over threshold
...
router.post('/remit', auth, async(req,res) =>{
  try {
    ...
    try {
      ...
      // chain failed, give back the inr + fee
      user.balance += amount + fee;
      await user.save();
      throw err;
    }
...
router.post('/zk/verify', auth,async(req,res)=> {
  try{
    ...
    let txHash = null;
    // ponytail: when no verifier contract deployed, proof is accepted off-chain
    if(config.zkVerifierAddress){
      ...
      txHash =await relayTx(config.zkVerifierAddress, data);
    }
...
// ponytail: DID routes — did:web, no registry, no contract
...
//ponytail: admin issues VC when KYC is verified
...
//ponytail: admin endpoints — basic email check, no role model needed yet
