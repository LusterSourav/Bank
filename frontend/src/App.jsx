...
function DashboardScreen({user,token, onSend,onDeposit,onHistory,onSettings,onKyc, onLogout,onWallet, onClaim }){
  ...
  //restore WebAuthn session from localStorage with 1hr expiry — survives page refresh
  ...
  // locks on tab switch, bit agressive but user asked
  useEffect(()=>{
  ...
  const verifyWebauthnUnlock= async () => {
    ...
    try{
      ...
      //  mediation:optional forces local biometric, no QR/hybrid fallback
      let authResp;
  ...
  const revealBalance= ()=>{
    ...
    privacyTimerRef.current =setTimeout(()=> setPrivacyRevealed(false),3000);//auto-hides balance after 3s
...
function SettingsScreen({user,onBack,onLogout,token,onKyc,onRefreshUser}){
  ...
  // device tracking, tried canvas fingerprinting first, too noisy
  useEffect(()=>{
  ...
  const saveWA=async()=>{
    ...
    try{
      const d=await apiFetch('/notifications/prefs',token,{method:'POST',body:JSON.stringify({phone,notifyWhatsApp:waEnabled})});
      setWaStatus('saved');
      localStorage.setItem('phone',d.phone);
      localStorage.setItem('waEnabled',d.notifyWhatsApp);
    }catch(e){setWaStatus('error')}
    setSavingWA(false);
  };

  useEffect(()=>{
    apiFetch('/notifications/history',token).then(d=>setNotifHistory(d||[])).catch(()=>{});
  },[token]);
        ...
        <div className="settings-card">
          <div className="settings-row" style={{flexDirection:'column',alignItems:'stretch',gap:'var(--sp-8)'}}>
            ...
            <div style={{display:'flex',alignItems:'center',gap:'var(--sp-8)'}}>
              <Toggle checked={waEnabled}onChange={()=>{const n=!waEnabled;setWaEnabled(n);localStorage.setItem('waEnabled',n)}}/>
              <span className="settings-row-value">{t('waSendAlerts')}</span>
            </div>
            <button className="settings-save-btn" onClick={saveWA}disabled={savingWA || !phone}>
              {savingWA ? t('saving') : waStatus === 'saved' ? t('saved') : waStatus === 'error' ? t('error') : t('save')}
            </button>
          </div>
          {notifHistory.length > 0 && (
            <>
              ...
              <div className="settings-row">
                <span className="settings-row-label">{t('notificationHistory')}</span>
              </div>
              {notifHistory.slice(0,4).map(n =>(
                <div key={n.id} className="settings-row" style={{opacity:0.8}}>
                  <div className="settings-row-left">
                    <span className={`notif-status-dot ${n.status}`}/>
                    <div>
                      <span className="settings-row-value">{n.type} ({n.channel})</span>
                      <span className="settings-row-value" style={{fontSize:'11px'}}>{new Date(n.createdAt).toLocaleDateString()}</span>
                    </div>
                  </div>
                </div>
              ))}
...
export default function App(){
    ...
    const unsub=onAuthStateChanged(auth, async (fbUser)=> {
      ...
      if (fbUser){
        ...
        try {
          ...
          const t= await fbUser.getIdToken();
          apiFetch('/device/fingerprint',t,{ method: 'POST',body: JSON.stringify({hash: deviceId})}).catch(() =>{});
        }catch{}
      }
      setLoading(false);
    });
  ...
  switch(screen){
    ...
    case 'dashboard': return <DashboardScreen user={user}token={token} onSend={() => setScreen('send')}onDeposit={() => setScreen('deposit')}onHistory={()=> setScreen('history')}onWallet={()=> setScreen('wallet')}onClaim={()=> setScreen('claim')}onSettings={()=> setScreen('settings')}onKyc={() => setScreen('kyc')}onLogout={handleLogout}/>;
